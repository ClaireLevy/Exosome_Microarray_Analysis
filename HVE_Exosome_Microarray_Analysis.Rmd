...
Plots of non-normalized data: HVE_A2 appears as the main outlier, it is donor A with no virus and low exosomes.
...
Test 1 uses the MediaVsSup contrast

```{r CAMERA setup test1}
#I'm going to do a CAMERA test using the MSigDB hallmark sets
...
index<-ids2indices(gene.sets = Hs.H, identifiers = identifiers)
```

```{r CAMERA test1}
#define a matrix of the contrast that you want the results for
contrMediaVsSup<-makeContrasts(TreatmentSeminalSupernatant - TreatmentMedia, levels = HVE_design)

#put arguments into camera
hallmarkMediaVsSup<-camera(y = eSet, index = index, design = HVE_design, contrast = contrMediaVsSup )

pander(head(hallmarkMediaVsSup))
```

Test 2 uses the MediaVsHighExos contrast

```{r CAMERA test2}
contrMediaVsHighExos<-makeContrasts(TreatmentHighExosomes - TreatmentMedia, levels = HVE_design)

#put arguments into camera
hallmarkMediaVsHighExos<-camera(y = eSet, index = index, design = HVE_design, contrast = contrMediaVsHighExos )

pander(head(hallmarkMediaVsHighExos))
```

Test 3 uses the MediaVsLowExos contrast

```{r CAMERA test2}
contrMediaVsLowExos<-makeContrasts(TreatmentLowExosomes - TreatmentMedia, levels = HVE_design)

#put arguments into camera
hallmarkMediaVsLowExos<-camera(y = eSet, index = index, design = HVE_design, contrast = contrMediaVsLowExos )

pander(head(hallmarkMediaVsLowExos))
```