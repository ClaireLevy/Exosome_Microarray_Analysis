...
Condition <-paste(targets$Virus, targets$Treatment, sep = ".")
...
LC_design <-model.matrix(~0+Condition+Donor)
```

```{r fit model and make contrasts}

LC_fit <- lmFit(LC_expressedProbes_lumibatch, design = LC_design)

LC_contrasts <- makeContrasts(
...
LC_fit2 <- contrasts.fit(LC_contrasts, fit = LC_fit)

LC_fit2 <- eBayes(LC_fit2)

```

```{r probes per contrast}

results <- decideTests(LC_fit2,method="separate", adjust.method="BH",
...
pander(ttNone.MediaVsNone.Sup %>%
  select(ProbeID, ENTREZ_GENE_ID,TargetID,DEFINITION, adj.P.Val))
```
...
ttHIV.MediaVsHIV.Sup <- topTable(LC_fit2, coef = "HIV.MediaVsHIV.Sup",
                  adjust.method = "BH", number=Inf, p.value=0.05, lfc=0.5)
...
eSet<-exprs(LC_expressedProbes_lumibatch)
...
identifiers = as.character(fData(LC_expressedProbes_lumibatch)[,"ENTREZ_GENE_ID"])
```
...
camera(y = eSet, index = index, design = LC_design, 
            contrast = LC_contrasts[,contrName])#subset the LC_contrasts matrix and get one contrast at a time.
  }
```
...
```{r None.MediaVsNone.Exos }
pander(head(hallMarkResults[["None.MediaVsNone.Exos"]]))
```

### None.Media Vs None.Lipo


```{r None.MediaVsNone.Lipo}

pander(head(hallMarkResults[["None.MediaVsNone.Lipo"]]))
```
...
```{r None.MediaVsNone.Sup }

pander(head(hallMarkResults[["None.MediaVsNone.Sup"]]))
```
...
```{r Sendai.MediaVsSendai.Exos}
pander(head(hallMarkResults[["Sendai.MediaVsSendai.Exos"]]))
```

### Sendai.Media Vs Sendai.Lipo

```{r Sendai.MediaVsSendai.Lipo}

pander(head(hallMarkResults[["Sendai.MediaVsSendai.Lipo"]]))
```

### Sendai.Media Vs Sendai.Sup

```{r Sendai.MediaVsSendai.Sup }

pander(head(hallMarkResults[["Sendai.MediaVsSendai.Sup"]]))
```

### HIV.Media Vs HIV.Exo

```{r HIV.MediaVsHIV.Exos}
pander(head(hallMarkResults[["HIV.MediaVsHIV.Exos"]]))
```


### HIV.Media Vs HIV.Lipo

```{r HIV.MediaVsHIV.Lipo}

pander(head(hallMarkResults[["HIV.MediaVsHIV.Lipo"]]))
```


### HIV.Media Vs HIV.Sup

```{r HIV.MediaVsHIV.Sup }

pander(head(hallMarkResults[["HIV.MediaVsHIV.Sup"]]))

pandoc.table(head(hallMarkResults[["Sendai.MediaVsSendai.Sup"]]))
```
